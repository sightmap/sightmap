...
| `sightmap browser explore --goal "..." --done-when view=Cart --value user=alice` | Drive the tab toward a goal by itself: every step observes the annotated tree, offers the interactive nodes as named options, and a typed picker (Jev by default, `TYPESAFE_API_KEY`; `--picker anthropic` for Claude) chooses one and judges "done". Typed values come only from `--value`/`--spec`. Add `--avoid Delete --avoid Pay` on real accounts. `--bench SUITE.json` runs a goal suite and prints a results table. Exits non-zero when the goal is not reached. |
