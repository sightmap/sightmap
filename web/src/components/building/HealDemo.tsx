...
export default function HealDemo() {
  const s = useShared()
  const walker = usePersonSlot(TRAVELLER_COLORS.test)
  ...
  const was = useMemo(() => new THREE.Vector3(), [])
  ...
  useFrame(({ clock }) => {
    ...
    // Walker position along the two legs. The crowd's gait driver measures the
    // travel between frames, so the walk cycle follows from this on its own.
    {
      let p = from
      if (phase === 'walk') {
      ...
      const dx = p.x - was.x
      const dz = p.z - was.z
      if (dx * dx + dz * dz > 1e-8) walker.ry = Math.atan2(dx, dz)
      was.copy(p)
      walker.x = p.x
      walker.y = p.y
      walker.z = p.z
