...
## P6.0 Feasibility spike — **gate on this**

Throwaway branch, no merge. Everything downstream assumes the lift is real and
affordable; this is where we find out.

- Target `sightmap-org` only. Our own site, `self-hosted` tier in effect, no
  third-party exposure while the harness is unproven.
- Two scenarios (one `navigate`, one `extract`), two variants, 3 repetitions.
- Answer, with numbers: does `@map` beat `@cold` above run-to-run spread? Which
  dimensions moved? What did 12 jobs cost in tokens and wall-clock? Does the
  Chrome-once bootstrap actually hold across isolated `HOME`s? Does a coding agent
  in an AXIS workspace drive `sightmap browser` without hand-holding?
- Deliverable: a written go/no-go with the raw `report.json` attached and, if
  no-go, what would have to change. **A weak or noise-level lift here is a
  legitimate outcome and stops the phase.**
...
- `axis/bin/bootstrap-chrome.sh` — `beforeAll` installs Chrome for Testing once
  into a shared path; per-job setup symlinks `$HOME/.sightmap/browsers` at it.
- `axis/fixtures/` for anything a scenario stages with `copy`.
...
- Acceptance: `npx axis run --scenario <fixture>` green from a clean checkout on a
  machine that has never installed Chrome, with both variants completing.
...
- `schema/axis-scenario.schema.json`: validates archetype membership, the two
  required variants, that `variants[].prompt` and `variants[].judge` are absent,
  and 3–5 scenarios per entry. Wire into `validate-entry` so a malformed scenario
  fails the same way a malformed corpus does.
...
- Hand-author scenarios for 3 seed entries as the reference set.
- Acceptance: an agent given only the skill and an entry slug produces scenarios
  that pass schema validation and a human review against SPEC §3.2 with no edits.

## P6.3 Result record + index — atlas

- `scripts/axis-record`: reads an AXIS `report.json`, computes medians, spreads,
  per-dimension lift, and `confidence` per SPEC §6, and writes
  `entries/<slug>/axis/result.json` + `history/<date>.json`. All six publishability
  gates enforced here, in code, not in review.
...
- Acceptance: fixture `report.json` in → exact expected `result.json` and index
  block out; a fixture with 2 repetitions, a self-judged run, a
...
- Card badge, detail scorecard, stale handling, sort behaviour, machine twins, and
  the `llms.txt` clause, all per SPEC §5.
...
- A methodology page on sightmap.org that is a fair reading of SPEC §2, §6, and §7,
  including what we expect *not* to move and why a negative lift gets published.
...
**Cost.** 4 scenarios × 2 variants × 5 repetitions is 40 agent runs per entry,
each driving a browser. This is the dominant constraint and the reason for a
schedule rather than per-merge scoring, hard limits, and a budget ceiling in
P6.4. If P6.0's measured cost makes a monthly full-atlas sweep untenable, score a
rotating subset and say so in the record.
...
scenarios and reports, published negatives, and the §6.6 goal-achievement gate are
all load-bearing. Dropping any one of them turns this from evidence into
marketing.
...
1. **Canonical agent and model.** One pair defines published scores. Claude Code
   is the obvious default; naming a second for research changes the cost model.
