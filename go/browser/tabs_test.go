...
// TestNewTab verifies that NewTab calls /json/new with the URL and connects via /json/list.
func TestNewTab(t *testing.T) {
	var mu sync.Mutex
	var newURLQuery string
	var listCalled bool
	...
	mux.HandleFunc("/ws", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			t.Logf("TestNewTab: ws upgrade: %v", err)
			return
		}
		var wsMu sync.Mutex
		go func() {
	...
	conn, err := NewTab(context.Background(), addr, "https://example.com")
	if err != nil {
		t.Fatal(err)
	}
	...
	mu.Lock()
	gotQuery := newURLQuery
	gotList := listCalled
	mu.Unlock()

	if !strings.Contains(gotQuery, "example.com") {
		t.Errorf("/json/new query=%q, expected to contain example.com", gotQuery)
	}
	if !gotList {
		t.Error("/json/list was not called during ConnectTab")
	}
}
