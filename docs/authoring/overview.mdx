...
`iterate` prints coverage and groups T3 nodes by a suggested selector. It writes a temporary capture without changing your saved view sets. Use `sightmap snapshot` when you need the full annotated tree or a saved capture.
...
Re-run `iterate` and work through the remaining T3 clusters. A route reaches the baseline when coverage reports `0 orphaned T3 ✓`. Then review its T2 scopes.
...
- Re-iterate affected pages. Coverage exposes new orphaned nodes while existing selectors and `memory` remain in the corpus.
...
- If you maintain saved view sets, refresh them with `sightmap snapshot --all`, then run `sightmap report`.
