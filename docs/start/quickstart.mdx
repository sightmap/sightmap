...
<Steps>
  ...
  <Step title="Have an agent build the corpus">
    ...
    > Build a sightmap for this app. Inspect the main routes with `sightmap iterate`, verify each selector with `sightmap sel-probe`, and name components until every view has zero orphaned T3 nodes. Let me review the YAML before committing it.
    ...
    `iterate` snapshots the page, reports coverage, and suggests selectors for unattributed nodes. The agent probes a selector, updates `.sightmap/`, and runs `iterate` again.
...
When a route or component changes, rerun `sightmap iterate` on the affected pages. Existing selectors and `memory` entries remain in the corpus, while coverage exposes new orphaned nodes.
