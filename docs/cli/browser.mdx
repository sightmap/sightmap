...
Drive the live tab toward a goal one step at a time. Each step observes the page as the annotated component tree, turns its interactive nodes into a short list of named actions, asks a picker which one to take (and whether the goal is already met), acts, waits for the page to settle, and repeats. The picker only ever answers typed questions: pick one of these, yes or no. It never invents text. Every value the loop types comes from `--value` or a spec file.

The default picker is [Jev](https://docs.typesafe.ai/introduction) through TypeSafe (`TYPESAFE_API_KEY`), which answers in about 200 ms, so a step costs a few hundred milliseconds with no big-model call. `--picker anthropic` puts Claude in the same seat for comparison (`ANTHROPIC_API_KEY`). `--plan` makes one Claude call up front to write the spec from the goal.

| Flag | Default | Description |
|------|---------|-------------|
| `--goal` | | What to achieve, in plain words (the positional args are joined into the goal too) |
| `--done-when` | | Deterministic finish check, repeatable and ANDed: `view=NAME`, `url=SUBSTR`, `text=SUBSTR`, `component=NAME`, `history=SUBSTR`, or `prop=Comp.name~value[@Within.name~value]` |
| `--value` | | `key=text` the loop may type (repeatable). Fields pick the value whose key overlaps their name; ties go to the picker |
| `--spec` | | JSON file with `done_when`, `values`, `hint`, and `avoid` |
| `--avoid` | | Drop controls whose name contains this (repeatable). Use it on real accounts: `Delete`, `Pay`, `Review changes` |
| `--picker` | `jev` | `jev[:model]` or `anthropic[:model]` |
| `--plan` | off | Ask Claude once to write the spec from the goal |
| `--max-steps` | `20` | Stop after this many steps |
| `--json` | off | Print the run (steps, timings, transitions) as JSON on stdout |
| `--bench` | | Run a suite JSON file instead of one goal; with `--repeat N`, `--only NAME`, `--out FILE` |
| `--grow` | off | Grow the corpus while exploring: name the unmapped controls of every page visited (see below) |

With no `--done-when` the loop stops when the picker's own "done" judgment passes 0.85. With one, the picker's judgment is only logged. Progress goes to stderr, one line per step with the picked element, the top probabilities, and where the time went:

```bash
sightmap browser explore --url https://www.saucedemo.com/ \
...
Pages with more than 60 candidates are folded into one option per owning component or landmark, plus up to 20 elements whose name or link matches a word of the goal; a group answer triggers a second pick inside that group. A control acted on twice at the same URL is hidden from later picks. `browser explore` exits non-zero when the goal is not reached.

`--bench` runs a suite of goals with a reset between them and prints a table plus one summary line (success, steps, seconds per step, model calls and latency, tokens, and a USD estimate for priced pickers). The suites under `go/explore/bench/` reproduce the numbers in that directory's README.

#### `--grow`: build the corpus as you go

With `--grow`, every page the loop observes also grows the corpus at `--sightmap-dir` (created with an empty `components.yaml` if absent). Unmapped interactive nodes are grouped by their nearest stable ancestor selector (the same hooks `explain` prints), Jev classifies each group as buttons, links, inputs, a select, a nav, cards, or noise, a template names it (`NavListLink`, `ProductPodLink`, `SearchProductsInput`), the scoped selector is verified against the offline matcher on that page, and the component is appended to the view's YAML with `label` (and `href` for links) properties. A page with no matching view gets one, named from its route pattern (`/catalogue/*/index.html` becomes `CatalogueDetail`). A selector that turns up on a second view is moved to `components.yaml` as a global. Every write is followed by a corpus reload and validation and is rolled back if the corpus stops loading; the running loop picks up the new corpus at once.

```bash
sightmap browser explore --sightmap-dir .sightmap --grow \
  --url https://books.toscrape.com/ --goal "Open the Travel category" --done-when url=category/books/travel
```

On books.toscrape.com, three passes over the ten-goal suite took an empty corpus to seven components and four views with every visited page fully covered, `validate` and `lint` clean, in about a second of grow time. Names are template names: hand the result to an authoring pass for better ones and memory lines.
