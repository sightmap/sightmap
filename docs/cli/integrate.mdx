...
Three verbs over the community atlas ([github.com/sightmap/atlas](https://github.com/sightmap/atlas)), the catalog of corpora other people have published:
...
Try the product name or a category, browse https://github.com/sightmap/atlas, or map the site yourself with sightmap init.
```

Matching is case-insensitive substring containment in either direction, so `pos` finds `square-pos` and `square-pos-android` finds it too. Results are ordered by how they matched (exact domain, exact slug, domain, slug, name, category, description), with ties broken alphabetically.
...
```json
{
  ...
  "index": {
    "source": "https://raw.githubusercontent.com/sightmap/atlas/main/index.json",
...
acme-shop  Acme Shop
  ...
  shop.acme.test · commerce · 3 views, 11 components · verified 2026-05-20
...
| Flag | Default | Description |
| --- | --- | --- |
| `--source` | `https://raw.githubusercontent.com/sightmap/atlas/main/entries/{slug}.tar.gz` | Archive URL template, with `{slug}` substituted (for mirrors and tests) |
...
```text
sightmap atlas: no corpus published for this slug: "no-such-thing" — browse https://github.com/sightmap/atlas, or search for the site: sightmap atlas find <domain>
```
