...
<Steps>
  ...
  <Step title="Lint for style">
    Run `sightmap lint --warn-only` and read the warnings as a review checklist. Once you have captures, lint auto-reconciles against them, so match counts separate real problems from false positives.
  </Step>
