...
<Note>
**Selector subset.** Sightmap selectors support the descendant (space) and direct-child (`>`) combinators only, and exactly four pseudo-classes: `:not()`, `:is()`, `:where()`, and `:has()`. The offline matcher evaluates `:has()` nested inside `:not()` or `:is()` (for example `div:not(:has(div))` to match only the innermost container) and descendant or child combinators inside `:not()` (for example `button:not(nav button)`). Sibling combinators (`+`, `~`), a top-level comma, and positional pseudo-classes such as `:nth-child()` are parse errors at `validate` time.
</Note>
...
| `--url` | — | Accepted but currently ignored: `suggest` scans the page already loaded in the tab. Navigate first with `sightmap browser navigate` |
...
| `--wait` | `0` | Extra seconds to wait before extracting |
...
When every visible interactive node is covered, the command prints `✓ no orphaned interactive nodes`, followed by a `⚠` line when only global components cover the page (no view models it). In scoped mode the header becomes `T2 gaps in [ComponentName] — N unmatched interactive elements:` and a clean result prints `✓ No T2 gaps in [ComponentName]`.
